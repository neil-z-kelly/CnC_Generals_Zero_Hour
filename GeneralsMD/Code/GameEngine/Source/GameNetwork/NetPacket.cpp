...
NetCommandMsg * NetPacket::readWrapperMessage(UnsignedByte *data, Int &i, Int dataAvailable) {
	...
	if (!isRoomInBuffer(i, dataAvailable, headerSize)) {
		DEBUG_LOG(("NetPacket::readWrapperMessage - packet is too short to hold a wrapper header, discarding the rest of the packet\n"));
		i = dataAvailable;
		return msg;
	}
	...
	if (!isRoomInBuffer(i, dataAvailable, dataLength)) {
		DEBUG_LOG(("NetPacket::readWrapperMessage - chunk data length %d is bigger than the %d bytes left in the packet, discarding the rest of the packet\n",
...
NetCommandMsg * NetPacket::readFileMessage(UnsignedByte *data, Int &i, Int dataAvailable) {
	...
	if ((i >= dataAvailable) || (data[i] != 0)) {
		DEBUG_LOG(("NetPacket::readFileMessage - filename isn't terminated within the packet, discarding the rest of the packet\n"));
		i = dataAvailable;
		return msg;
	}
	++i;
	...
	if (!isRoomInBuffer(i, dataAvailable, sizeof(UnsignedInt))) {
		DEBUG_LOG(("NetPacket::readFileMessage - packet is too short to hold the file data length, discarding the rest of the packet\n"));
		i = dataAvailable;
		return msg;
	}
	...
	if (!isRoomInBuffer(i, dataAvailable, dataLength)) {
		DEBUG_LOG(("NetPacket::readFileMessage - file data length %d is bigger than the %d bytes left in the packet, discarding the rest of the packet\n",
			dataLength, dataAvailable - i));
		i = dataAvailable;
		return msg;
	}

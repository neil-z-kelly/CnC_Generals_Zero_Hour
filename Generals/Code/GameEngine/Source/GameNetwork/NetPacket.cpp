...
NetCommandRef * NetPacket::ConstructNetCommandMsgFromRawData(UnsignedByte *data, UnsignedShort dataLength) {
	...
	while ((offset < (Int)dataLength) && notDone) {
		if (data[offset] == 'T') {
			...
			if (msg == NULL) {
				DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("NetPacket::ConstructNetCommandMsgFromRawData - could not read a command of type %d\n", commandType));
				return NULL;
			}
...
NetCommandMsg * NetPacket::readFileMessage(UnsignedByte *data, Int &i, Int dataAvailable) {
	...
	if ((i >= dataAvailable) || (data[i] != 0)) {
		...
		DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("NetPacket::readFileMessage - unterminated filename, discarding message\n"));
		i = dataAvailable;
		msg->detach();
		return NULL;
	}
	...
	if ((dataAvailable - i) < (Int)sizeof(dataLength)) {
		DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("NetPacket::readFileMessage - truncated file data length, discarding message\n"));
		i = dataAvailable;
		msg->detach();
		return NULL;
	}
	...
	if (dataLength > (UnsignedInt)(dataAvailable - i)) {
		...
		DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("NetPacket::readFileMessage - file data length %d exceeds the %d bytes remaining, discarding message\n", dataLength, dataAvailable - i));
		i = dataAvailable;
		msg->detach();
		return NULL;
	}

...
NetCommandMsg * NetPacket::readWrapperMessage(UnsignedByte *data, Int &i, Int packetLen) {
	...
	if ((i < 0) || (packetLen < i) || ((packetLen - i) < headerSize)) {
		DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("NetPacket::readWrapperMessage - truncated wrapper header, discarding the rest of the packet\n"));
		i = packetLen;
		return NULL;
	}
	...
	if ((dataLength > bytesLeft) ||
		...
		DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("NetPacket::readWrapperMessage - bad chunk description, discarding the rest of the packet\n"));
		i = packetLen;
		return NULL;
	}
...
NetCommandMsg * NetPacket::readFileMessage(UnsignedByte *data, Int &i, Int packetLen) {
	...
	if ((i >= packetLen) || (data[i] != 0) || ((packetLen - (i + 1)) < (Int)sizeof(UnsignedInt))) {
		DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("NetPacket::readFileMessage - truncated file message, discarding the rest of the packet\n"));
		i = packetLen;
		return NULL;
	}
	++i;
	...
	if ((dataLength > (UnsignedInt)(packetLen - i)) || (dataLength > MAX_WRAPPED_COMMAND_SIZE)) {
		DEBUG_LOG_LEVEL(DEBUG_LEVEL_NET, ("NetPacket::readFileMessage - file data length %d exceeds the received packet, discarding the rest of the packet\n", dataLength));
		i = packetLen;
		return NULL;
	}
